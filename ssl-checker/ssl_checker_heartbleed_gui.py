...
# Heartbleed check function
def check_heartbleed(host: str, port: int) -> bool:
    ...
    try:
        ...
        tk_messagebox.showerror("Error", f"Could not connect to {host}:{port}\n{str(e)}")
        return False
    ...
    try:
        payload_length = random.randint(1, 65535)  # Random payload length
        payload = b'A' * payload_length  # Random payload
        heartbeat_request = struct.pack('!BHH', 1, payload_length, len(payload)) + payload

        s.send(heartbeat_request)
        response = s.recv(2048)

        if len(response) > payload_length + 3:  # 3 bytes for Heartbeat response header
            return True  # Vulnerable to Heartbleed
        else:
            return False  # Not vulnerable to Heartbleed
    except Exception as e:
        tk_messagebox.showerror("Error", f"An error occurred during Heartbeat check:\n{str(e)}")
        return False
    finally:
        s.close()

# Heartbleed check callback
def check_heartbleed_callback() -> None:
    host = host_strvar.get()
    port = int(port_strvar.get())

    result = check_heartbleed(host, port)
    if result:
        tk_messagebox.showinfo("Heartbleed Check", f"{host}:{port} is vulnerable to Heartbleed!")
    else:
        tk_messagebox.showinfo("Heartbleed Check", f"{host}:{port} is NOT vulnerable to Heartbleed.")
...
# New button for checking Heartbleed vulnerability
heartbleed_button = ttk.Button(
    main_frame, text="Check Heartbleed Bug", command=lambda: Thread(target=check_heartbleed_callback).start()
)

# Place the existing widgets
...
heartbleed_button.grid(column=0, columnspan=2, row=6, pady=(10, 0), sticky=tk.EW)  # Place the new button
